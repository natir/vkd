...
def read_parquet(input_directory: pathlib.Path, chr_name: str, config: dict[str, typing.Any]) -> polars.LazyFrame:
    ...
    lf = lf.rename(
        {key: name for key, name in config["alias"].items() if key in config["alias"] and key in schema.names()},
    )
...
def filter_and_collect(
    _lf: polars.LazyFrame,
    cols_values: list[tuple[str, typing.Any]],
    fraction: int,
) -> polars.DataFrame:
    """Filter on column and collect lazyframe."""
    return collect_and_sample(
...
@streamlit.cache_data
def one_column(df: polars.DataFrame, col_select: str, keep_col: list[str]) -> polars.DataFrame:
    """Perform select of column, mainly here for cache."""
    return df.select(*keep_col, col_select)
...
def line_chart(
    ...
    return (
        ...
        .encode(
            x=altair.X(x),
            y=altair.Y(y).scale(type=y_scale),
            color=altair.Color(color),
            opacity=altair.when(selection).then(altair.value(1)).otherwise(altair.value(0.1)),
        )
        .add_params(
...
def violin_chart(df: polars.DataFrame, col_select: str, color: str) -> altair.Chart:
    ...
    return (
        ...
        .encode(
            ...
            altair.Y(f"{col_select}:Q"),
            altair.Color(f"{color}:N"),
            altair.Column(f"{color}:N").spacing(0).header(titleOrient="bottom", labelOrient="bottom", labelPadding=0),
            opacity=altair.when(selection).then(altair.value(1)).otherwise(altair.value(0.1)),
        )
        .configure_view(
            stroke=None,
