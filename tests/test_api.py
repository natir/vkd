...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `vkd`."""
    not_exposed = [
        obj.path for obj in modulelevel_internal_objects if obj.name not in vkd.__all__ or not hasattr(vkd, obj.name)
    ]
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("vkd")
    for item in inventory.values():
        if item.domain == "py" and "(" not in item.name and (item.name == "vkd" or item.name.startswith("vkd.")):
